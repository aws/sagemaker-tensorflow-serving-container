...
- `model_version (string)`: version of the model, for example 'v1'
...
- `content_length (string)`: content length of the original request
...
If you are not using SageMaker Python SDK, you should package the contents in model directory (including models, inference.py and external modules) in .tar.gz format and upload it to S3. [Here](https://github.com/aws/sagemaker-python-sdk/blob/master/src/sagemaker/fw_utils.py#L146) is how we tar and upload the source directory to a S3 bucket. You can have other ways to do this but please make sure that the SageMaker Model entity has to point the compressed tar file in S3 as its model data URL.
